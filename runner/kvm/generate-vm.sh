...
generate_debian() {
    ...
    virt-builder "debian-${version}" \
        ...
        --run-command "DEBIAN_FRONTEND=noninteractive apt-get install -y curl gpg" \
        --run-command "DEBIAN_FRONTEND=noninteractive apt-get upgrade -y" \
        ...
        --run-command "apt-get update" \
        --run-command "DEBIAN_FRONTEND=noninteractive apt-get install -y sudo coreutils dpkg-dev debootstrap git python3-sh wget rpm devscripts rsync python3-packaging createrepo-c python3-yaml docker.io python3-docker reprepro python3-pathspec mktorrent openssl tree python3-setuptools python3-lxml gitlab-runner git-lfs openssh-server" \
        --run-command "git lfs install --skip-repo" \
        --ssh-inject "gitlab-runner:file:$ssh_pub_key" \
        --run-command "groupmod -g 11000 gitlab-runner" \
        --run-command "rm -f /root/.ssh/known_hosts" \
